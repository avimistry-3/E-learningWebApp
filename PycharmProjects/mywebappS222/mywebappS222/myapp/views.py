...
from .models import Topic, Course, Student, Order,Document
...
def details(request, top_no):
    ...
    top_courses = Course.objects.filter(topic__id=top_no)
...
def myaccount(request):
    ...
    if request.user.has_perm("myapp.canSeeMyAccount"):
        ...
        student = Student.objects.get(student_name=full_name)
        # print(student)
        # print(student.Image)
        # print(student.Image)
        ...
        return render(request, 'myapp/my_account.html',
                      {"first_name": first_name, "last_name": last_name, "orders": orders, "topics": topics,
                       "fname": fname, 'img': student.Image, 'imageExists': imgExists})
       # return render(request, 'myapp/my_account.html',
        #              {"first_name": first_name, "last_name": last_name, "orders": orders, "topics": topics,
         #              "fname": fname})
    else:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid(): 
                ...
                if int(isStudent) == 1:
                    ...
                    newStudent = Student.objects.create(student_name=str(firstName)+" "+str(lastName))
                    print(user.username)
